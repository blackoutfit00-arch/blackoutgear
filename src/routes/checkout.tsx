...
function CheckoutPage() {
  ...
  const fullAddress = [
    ...
    houseNo.trim() && `House ${houseNo.trim()}`,
  ]
    .filter(Boolean)
    .join(", ");
  ...
  const isAddressValid =
    block.trim().length > 0 && road.trim().length > 0 && houseNo.trim().length > 0;
  ...
  const placeShopifyOrder = async () => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        toast.error("Shopify rejected the order", { description: result.error, duration: 9000 });
        setIsPlacingOrder(false);
        return;
      }

      if (!result.orderName) {
        toast.error("Shopify rejected the order", {
          description: "Shopify returned no order name. Please try again.",
          duration: 9000,
        });
        setIsPlacingOrder(false);
        return;
      ...
      saveLastOrder({
        ...
        name: name.trim(),
        phone: phone.trim(),
        address: `${region.trim()}, ${fullAddress}`,
        currency,
        subtotal,
        discountPercent,
        discountAmount,
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 space-y-4 px-4 py-8">
        ...
        <SectionCard title="Delivery Address">
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                <label className="mb-1.5 block text-sm text-muted-foreground">
                  Road <span className="text-destructive">*</span>
                </label>
                <Input
                  value={road}
          ...
          <div className="flex items-center gap-3 rounded-lg border-2 border-accent bg-accent/10 p-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Payment details are shown after you confirm your order
              </p>
            </div>
          </div>
        </SectionCard>
        ...
        <div className="flex items-start gap-2">
          ...
          <label htmlFor="agree" className="text-xs leading-relaxed text-muted-foreground">
            ...
            and confirm my personal data will be used to process and deliver my order. Delivery
            takes 8–15 days from order confirmation.
          </label>
        </div>
        <Button
          ...
          size="lg"
          className={`label-caps h-14 w-full rounded-xl text-base transition-colors ${isFormComplete ? "bg-accent text-accent-foreground hover:bg-accent/90" : "bg-primary text-primary-foreground hover:bg-primary/90"}`}
        >
