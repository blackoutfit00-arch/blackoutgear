  ...
  return (
    <div className="min-h-screen bg-[#f4efe5] text-[#122b4b]">
        ...
        <section className="relative overflow-hidden bg-[#f4efe5]">
          <div className="mx-auto grid min-h-[560px] max-w-[1500px] items-center gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[1fr_0.9fr] lg:py-20">
            <div className="max-w-xl">
              <p className="mb-5 text-[11px] font-semibold uppercase tracking-[0.28em] text-[#8b6b45]">
                Premium eyewear · Bahrain
              </p>
              ...
              <p className="mt-7 max-w-md text-base leading-7 text-[#122b4b]/70 sm:text-lg">
                Premium sunglasses and optical frames crafted for everyday clarity, comfort and
                effortless style.
              </p>
              <a
                href="#shop"
                className="mt-9 inline-flex items-center gap-3 bg-[#122b4b] px-7 py-4 text-xs font-semibold uppercase tracking-[0.18em] text-[#f5efe2] transition-transform hover:-translate-y-0.5"
              >
            ...
            <div className="relative mx-auto w-full max-w-[600px]">
              ...
              {featured[0]?.node.images?.edges?.[0]?.node.url ? (
                <img
                  src={featured[0].node.images.edges[0].node.url}
                  alt={featured[0].node.title}
                  className="relative aspect-square w-full object-cover mix-blend-multiply"
                />
              ) : (
      ...
      <main id="shop" className="mx-auto max-w-[1500px] px-5 py-14 sm:px-8 sm:py-20">
        {q && q.trim() && (
          <p className="mb-8 text-sm text-[#122b4b]/60">
            Results for <span className="font-semibold text-[#122b4b]">"{q.trim()}"</span>
          </p>
        )}

        <div className="mb-10 flex items-end justify-between border-b border-[#d9d0c0] pb-5">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-[#8b6b45]">
              The collection
            </p>
          ...
          <p className="hidden text-xs uppercase tracking-[0.16em] text-[#122b4b]/50 sm:block">
            Built for Bahrain
          </p>
        </div>
        ...
        {isLoading ? (
          <div className="flex justify-center py-24">
            <Loader2 className="h-6 w-6 animate-spin text-[#122b4b]/50" />
          </div>
        ) : isError ? (
          <p className="py-24 text-center text-[#122b4b]/60">
