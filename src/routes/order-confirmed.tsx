...
export const Route = createFileRoute("/order-confirmed")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: `Your order at ${STORE_NAME} has been placed. Transfer to our IBAN and send the receipt on WhatsApp to confirm.`,
      },
      { property: "og:title", content: `Order Confirmed — ${STORE_NAME}` },
      {
        property: "og:description",
        content: "Your order has been placed. Send your transfer receipt on WhatsApp to confirm.",
      },
...
function OrderConfirmedPage() {
  ...
  const sendWhatsApp = () => {
    const text = order
      ...
      : `Hi ${STORE_NAME}! Here is my transfer receipt.`;
      ...
      <main className="mx-auto w-full max-w-md flex-1 space-y-6 px-4 py-10 text-center">
        ...
        <div className="space-y-4 rounded-xl border border-accent/40 bg-accent/10 p-4 text-left">
          <p className="text-sm leading-relaxed text-muted-foreground">
            To confirm your order, transfer the amount to the IBAN below via BenefitPay, then send
            us a photo of the transfer receipt on WhatsApp.
          </p>
