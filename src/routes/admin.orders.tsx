    ...
    body = (
      <div className="mx-auto max-w-sm space-y-3 rounded-xl border border-border bg-card p-4">
        ...
        <div className="flex gap-2">
          <Button className="label-caps flex-1" onClick={() => auth("in")}>
            Sign in
          </Button>
          <Button variant="outline" className="label-caps flex-1" onClick={() => auth("up")}>
            Sign up
          </Button>
        </div>
      </div>
    ...
    body = (
      <div className="space-y-3 text-center">
        <p className="text-muted-foreground">
          This account ({session.user.email}) doesn't have admin access yet.
        </p>
      ...
      <div className="space-y-3">
        ...
        {orders.map((o) => {
          const items = Array.isArray(o.items)
            ? (o.items as { title: string; options?: string; quantity: number }[])
            : [];
          return (
            <div
              key={o.id}
            ...
            >
              ...
              <ul className="space-y-0.5">
                {items.map((i, idx) => (
                  <li key={idx}>
                    • {i.title}
                    {i.options ? ` · ${i.options}` : ""} × {i.quantity}
                  </li>
                ))}
              </ul>
              <div className="flex items-center justify-between border-t border-border pt-2">
                ...
                <span className="text-lg font-bold">
                  {formatMoney(Number(o.total_price), o.currency)}
                </span>
              </div>
            </div>
          );
