...
async function adminGraphQL(token: string, query: string, variables: Record<string, unknown>) {
  ...
  if (!res.ok)
    throw new Error(
      `Shopify HTTP ${res.status}: ${json?.errors?.[0]?.message || "Request rejected"}`,
    );
...
async function createRealOrder(
  ...
  try {
    const result = await adminGraphQL(token, query, {
      order: input,
      options: {
    ...
    return order?.name
      ? { ok: true, orderName: order.name }
      : { ok: false, error: "Shopify returned no order." };
  } catch (e) {
    return {
      ok: false,
      error: e instanceof Error ? e.message : "Shopify Admin API request failed.",
    };
  }
}
...
export const createShopifyOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ShopifyOrderResult> => {
    ...
    if (!token)
      return { ok: false, error: "SHOPIFY_ADMIN_ACCESS_TOKEN is missing in Vercel Production." };
